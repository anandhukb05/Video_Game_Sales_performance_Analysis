## Insights Summary with Visualizations

This section summarizes the key findings from the analysis with corresponding charts.

---

### 1. Sales Analysis

#### 🔹 Top 10 Best-Selling Games (Global)
![Top Selling Games](reports/visuals/top_10_best_selling_games.png)

#### 🔹 Regional Sales Comparison
Bar chart comparing average sales across regions.
![Regional Sales Comparison](reports/visuals/sales_by_region_bar.png)

#### 🔹 Genre Popularity
Top-performing genres globally.
![Genre Popularity](reports/visuals/genre_popularity.png)

#### 🔹 Japanese vs. North American Sales (Top Games)
Comparison of regional sales.
![JP vs NA Sales](reports/visuals/jp_vs_na_sales.png)

---

### 2. Critic Score Impact

#### 🔹 Correlation: Critic Score vs. Total Sales
Visualizing relationship strength between critic ratings and sales.
![Critic vs Sales Correlation](reports/visuals/critic_vs_sales_correlation.png)

#### 🔹 Top Publishers by Average Critic Score
![Publisher Ratings](reports/visuals/publisher_critic_score.png)

---

### 3. Time-Based Trends

#### 🔹 Yearly Global Sales Trend (2000–2024)
Identifying peak years and blockbuster trends.
![Yearly Sales Trend](reports/visuals/yearly_sales_trend.png)

#### 🔹 Regional Sales by Year
Shows how each region’s market evolved over time.
![Regional Sales by Year](reports/visuals/region_wise_sales_by_year.png)

---

### 4. Regional Insights

#### 🔹 Genre Preferences by Region
Understanding different gaming tastes.
![Regional Genre Preference](reports/visuals/genre_by_region.png)

#### 🔹 Top Publishers in Each Region
![Regional Publisher Performance](reports/visuals/regional_publisher_performance.png)

---

### 5. Publisher / Developer Performance

#### 🔹 Average Sales per Publisher
![Publisher Performance](reports/visuals/publisher_performance.png)

#### 🔹 Developer vs. Average Critic Score
![Developer Ratings](reports/visuals/developer_critic_score.png)

---

### 6. Platform Evolution

#### 🔹 Top 10 Multi-Platform Sales Comparison
Compare how the same title performed on different consoles.
![Platform Comparison](reports/visuals/platform_sales_comparison.png)

#### 🔹 Console Lifecycle Trends
Which platforms dominated each era.
![Platform Lifecycle](reports/visuals/console_lifecycle.png)

#### 🔹 Correlation Heatmap: Sales and Critic Score by Region
![Correlation Heatmap](reports/visuals/correlation_heatmap.png)


